...
# ====================================================================
# TEST: Pagina principal (Home)
# ====================================================================

@pytest.mark.functional
class TestHomePage:
    def test_home_page_loads(self, server, driver, screenshot):
        driver.get("http://127.0.0.1:9901/")
        time.sleep(0.5)
        screenshot("02_pagina_cargada")
        body = driver.page_source.lower()
        assert "climapp" in body or "clima" in body

    def test_home_has_login_button(self, server, driver, screenshot):
        driver.get("http://127.0.0.1:9901/")
        time.sleep(0.5)
        screenshot("02_pagina_con_boton_login")
        body = driver.page_source.lower()
        assert "login" in body or "iniciar" in body or "auth0" in body

    def test_home_title(self, server, driver, screenshot):
        driver.get("http://127.0.0.1:9901/")
        time.sleep(0.5)
        screenshot("02_titulo_verificado")
        assert driver.title is not None

    def test_home_has_css_styles(self, server, driver, screenshot):
        driver.get("http://127.0.0.1:9901/")
        time.sleep(0.5)
        screenshot("02_estilos_css_cargados")
...
# ====================================================================
# TEST: Redirecciones de autenticacion
# ====================================================================
...
class TestAuthenticatedPages:
    ...
    def test_sidebar_present_when_authenticated(
        ...
        _login_via_cookie(server, driver, admin_user)
        screenshot("02_sesion_iniciada_verificando_sidebar")
        driver.get("http://127.0.0.1:9901/dashboard")
        time.sleep(1.0)
        screenshot("03_sidebar_visible")
        body = driver.page_source.lower()
    ...
    def test_usuarios_page_loads_for_admin(
        ...
        _login_via_cookie(server, driver, admin_user)
        screenshot("02_sesion_admin_navegando_usuarios")
        driver.get("http://127.0.0.1:9901/dashboard/usuarios")
        time.sleep(1.0)
        screenshot("03_pagina_usuarios_cargada")
        assert "/usuarios" in driver.current_url

    def test_usuarios_page_forbidden_for_viewer(
        self, server, driver, viewer_user, screenshot
    ):
        _login_via_cookie(server, driver, viewer_user)
        screenshot("02_sesion_viewer_intentando_acceder_usuarios")
        driver.get("http://127.0.0.1:9901/dashboard/usuarios")
        time.sleep(1.0)
        screenshot("03_acceso_denegado_viewer")
        body = driver.page_source.lower()
    ...
    def test_clima_page_loads_when_authenticated(
        ...
        _login_via_cookie(server, driver, admin_user)
        screenshot("02_sesion_admin_navegando_clima")
        driver.get("http://127.0.0.1:9901/dashboard/clima")
        time.sleep(1.0)
        screenshot("03_pagina_clima_cargada")
...
@pytest.mark.functional
class TestLogout:
    def test_logout_clears_session(self, server, driver, admin_user, screenshot):
        _login_via_cookie(server, driver, admin_user)
        screenshot("02_sesion_activa_antes_logout")
        driver.get("http://127.0.0.1:9901/dashboard")
        time.sleep(0.5)
        screenshot("03_dashboard_antes_de_cerrar_sesion")
        driver.get("http://127.0.0.1:9901/logout")
        time.sleep(0.5)
        screenshot("04_sesion_cerrada")
        ...
        screenshot("05_verificacion_sesion_invalida")
        assert resp.status_code in (301, 302)

    def test_logout_redirects_to_home(self, server, screenshot):
        screenshot("02_solicitando_logout")
        resp = http_requests.get(
            "http://127.0.0.1:9901/logout",
            allow_redirects=False,
        )
        screenshot("03_respuesta_logout_obtenida")
        assert resp.status_code in (301, 302)
        location = resp.headers.get("Location", "")
        assert "/" in location
