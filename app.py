...
def create_app(config_overrides=None):
    ...
    application.secret_key = os.getenv("SECRET_KEY", "fallback_secret")

    application.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///climapp.db"
...
# ==========================================================
    ...
    @application.route("/dashboard/clima")
    @login_required
    def clima():
        return render_template("sections/clima.html")

    @application.route("/dashboard/clima/pronostico")
    @login_required
    def pronostico():
        return render_template("sections/pronostico.html")

    @application.route("/dashboard/clima/historial")
    @login_required
    def historial():
        return render_template("sections/historial.html")

    @application.route("/dashboard/clima/alertas")
    @login_required
    def alertas():
        return render_template("sections/alertas.html")

    @application.route("/dashboard/usuarios")
    @login_required
    @role_required("admin")
    def usuarios():
    ...
    @application.route("/dashboard/config")
    @login_required
    def config():
        return render_template("sections/config.html")

    @application.route("/api/user/roles")
    @login_required
    def api_user_roles():
    ...
    @application.route("/api/users/<int:user_id>/role", methods=["PUT"])
    @login_required
    @role_required("admin")
    ...
    @application.route("/api/roles", methods=["GET"])
    @login_required
    def api_list_roles():
...
# ==========================================================
    ...
    def api_dashboard():
        ...
        total_ciudades = (
            db.session.query(HistorialConsulta.ciudad)
            .distinct().count()
        )
        ...
        consultas_por_ciudad = [
            {"ciudad": c.ciudad, "cantidad": c.cantidad} for c in ciudades
        ]
